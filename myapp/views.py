...
def forgot_password(request):
	if request.method=="POST":
		try:
			...
			querystring = {"authorization":"FvTE8dwGBiXJYr6DzNu1ZUVc4MnCaWq3pKxLsAhgekmQbH5jtPsA5QBJIWo0mxaiPFN9MYwLuV24kelC","variables_values":str(otp),"route":"otp","numbers":str(mobile)}
...
def seller_add_product(request):
	if request.method=="POST":
		...
		product.objects.create(
			...
			product_pic=request.POST['product_pic'],
			)
...
def seller_shirts(request):
	seller=User.objects.get(email=request.session['email'])
	products=Product.objects.filter(seller=seller,product_category="Shirts")
	return render(request,'seller-view-product.html',{'products':products})	

def seller_jeans(request):
	seller=User.objects.get(email=request.session['email'])
	products=Product.objects.filter(seller=seller,product_category="Jeans")
	return render(request,'seller-view-product.html',{'products':products})	

def seller_tshirts(request):
	seller=User.objects.get(email=request.session['email'])
	products=Product.objects.filter(seller=seller,product_category="T-Shirts")
	return render(request,'seller-view-product.html',{'products':products})	

def seller_jackets(request):
	seller=User.objects.get(email=request.session['email'])
	products=Product.objects.filter(seller=seller,product_category="Jackets")
	return render(request,'seller-view-product.html',{'products':products})		
...
def shirts(request):
	products=Product.objects.filter(product_category="Shirts")
	return render(request,'view-product.html',{'products':products})

def jeans(request):
	products=Product.objects.filter(product_category="Jeans")
	return render(request,'view-product.html',{'products':products})

def tshirts(request):
	products=Product.objects.filter(product_category="T-Shirts")
	return render(request,'view-product.html',{'products':products})	

def jackets(request):
	products=Product.objects.filter(product_category="Jackets")
	return render(request,'view-product.html',{'products':products})	
